...
```{r}
matches |> summarize(corelation = cor(fouls, goal_diff, method="pearson"))
```
...
There really was not a strong correlation between fouls and wins in soccer in either direction. The scatterplot is not very useful becasue the data doesn't fit a close line and is kind of just a big blob of data. 
