...
```{r}
crimes |> mutate(violent_crime_rate = violent_crime_total/population * 10000)
crimes |> mutate(property_crime__rate = property_crime_total/population * 10000)
```
...
There are far more property crimes per 10,000 people a year than violent crimes. However, as I expected, the rates for both crimes have dropped in a major way since 1975. Violent crimes are down over 50% since 1975, and property crimes are down about 33%. This matches what I expected.
