...
I would say yes, the F-Statistic indicates that as a while the model is valuable in some way. For some independent variables it might not be the greatest, but as a whole it will give us something. Overall, it's clearly not a perfect model, but it's not useless.
...
Points scored obviously has the most impact on winning. Yards gained, while correlated with points scored, is actually not in itself super valuable in itself. The correlation between yards gained and wins is actually the weakest out of any of the three variables, so I would look at just pure yards gained as not super imoactful.
...
The takeaway I would have from looking at this data to cover the NFL would actually be to look at teams with high yardage numbers compared to how much they are actually scoring. Are teams just gaining empty calories or actually putting points on the board. Also when you tweak the numbers to add the defensive statistics the model gets better. This leads me to believe that in general what is more successful in the NFL is allowing less yards and protecting the ball. I would say that this is because it's probably just harder in the NFL to stop teams from scoring than it is to score the ball yourself, so playing good defense and not turning it over is the best way to win.
