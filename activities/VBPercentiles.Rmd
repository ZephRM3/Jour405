...
```{r}
team_aces <- matches |> group_by(team) |> summarize(total_aces = sum(aces)) |> arrange(desc(total_aces))
```
...
Maryland was in the 95th percentile in the country this year for service aces in volleyball. For comparison Penn State, the national champion in 2024 and Big Ten rival was only in the 87th percentile. However, this may show that maybe service ace percentage is not that important of a stat.
